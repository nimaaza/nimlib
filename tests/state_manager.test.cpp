...
enum States { START, OK, ERROR, SOME_STATE, ANOTHER_STATE };

TEST(StateManagerTest_Construction, InitialState_SetToStart)
{
    StateManager<States> sm{ States::START, States::ERROR, 2 };
...
TEST(StateManagerTest_SetState, ResetCountZeroedOnStateChange)
{
    ...
    EXPECT_EQ(state3, States::ERROR);
}

TEST(StateManagerTest_SetState, StateNotNew)
{
    StateManager<States> sm{ States::START, States::ERROR, 3 };
    sm.set_state(States::SOME_STATE);
    ...
    EXPECT_EQ(state_after_limit, States::ERROR);
}
...
{
    StateManager<States> sm{ States::START, States::ERROR, 4 };
    ...
    EXPECT_EQ(state_after_reached, States::ERROR);
}
